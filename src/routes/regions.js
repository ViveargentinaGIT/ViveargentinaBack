...
router.post("/", async (req, res) => {
  const {name} = req.body;
  if (!name) {
    return res.status(201).send("You must enter a name to create the new region");
  } else {
    try {
      ...
      console.log(err);
      return res.status(404).send("An error ocurred in the creation of the region");
    }
  }
});
